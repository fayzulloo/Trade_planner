...
Arxitektura:
  trades          — asosiy jurnal (hech qachon o'chirmaydi)
  strategy_trades — strategiya jurnali (yangi strategiyada tozalanadi)
                    trades ga reference orqali ishlaydi (duplicate yo'q)
"""
        ...
        async with conn.transaction():
            ...
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS settings (
                    ...
                    withdrawal_every      INTEGER       DEFAULT 0,
            ...
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS trades (
                    ...
                    sl_price     NUMERIC(12,5) DEFAULT NULL,
                    tp_price     NUMERIC(12,5) DEFAULT NULL,
            ...
            # --- strategy_trades (strategiya jurnali) ---
            # trades ga reference — ma'lumot ikki marta saqlanmaydi
            # Yangi strategiya boshlananda: DELETE FROM strategy_trades WHERE user_id = X
            await conn.execute("""
                ...
                CREATE INDEX IF NOT EXISTS idx_strategy_trades_user
                    ON strategy_trades (user_id, day_number);
            """)
...
async def migrate_db() -> None:
    ...
    async with pool.acquire() as conn:
        ...
        migrations = [
            ...
            ("settings", "end_date",               "TEXT DEFAULT NULL"),
            ("settings", "mode",                   "TEXT DEFAULT 'strategy'"),
            # trades
        ...
        # strategy_trades jadvalini yaratish (migration sifatida)
        try:
            await conn.execute("""
                CREATE TABLE IF NOT EXISTS strategy_trades (
                    id         SERIAL PRIMARY KEY,
            ...
            logger.error(f"strategy_trades migration xato: {e}")
