...
async def save_settings(user_id: int, **kwargs) -> None:
    ...
    ⚠️ Xavfsizlik: faqat ALLOWED_SETTINGS da ro'yxatdagi
    maydonlar qabul qilinadi — SQL injection dan himoya.

    Misol: save_settings(user_id, starting_balance=1000, daily_profit_rate=0.10)
    """
    # Ruxsat etilgan maydonlar ro'yxati — SQL injection dan himoya
    ALLOWED_SETTINGS = {
        "starting_balance", "daily_profit_rate", "extra_target",
        "withdrawal_amount", "withdrawal_every", "total_days",
        "start_date", "end_date", "timezone", "reminder_time",
        "evening_reminder_time", "auto_complete_time", "broker_name",
        "rest_days", "is_active", "mode",
    }

    # Noma'lum maydonlarni filtrlash
    filtered = {k: v for k, v in kwargs.items() if k in ALLOWED_SETTINGS}
    if not filtered:
        return
...
async def add_trade(
    ...
    - trades (asosiy jurnal) ga doim yozadi
    - strategy_trades ga faqat mode='strategy' bo'lsa yozadi
    ...
    try:
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                # 2. Strategiya rejimida — strategy_trades ga ham yozish
                mode_row = await conn.fetchrow(
                    "SELECT mode FROM settings WHERE user_id = $1;", user_id
                )
                if mode_row and mode_row["mode"] == "strategy":
                    await conn.execute("""
                        INSERT INTO strategy_trades (user_id, trade_id, day_number)
                        VALUES ($1, $2, $3)
                        ON CONFLICT (user_id, trade_id) DO NOTHING;
                    """, user_id, trade["id"], day_number)

                # 3. Bugungi journal actual_pnl ni yangilash
                await conn.execute("""
...
async def create_journal_day(
    ...
    try:
        async with pool.acquire() as conn:
            ...
            # ON CONFLICT DO NOTHING bo'lsa RETURNING None qaytaradi
            # — mavjud yozuvni to'g'ridan-to'g'ri o'qib qaytaramiz
            if row is None:
                row = await conn.fetchrow("""
                    SELECT * FROM daily_journal
                    WHERE user_id = $1 AND date = $2;
...
async def finish_strategy(user_id: int, end_date_str: Optional[str] = None) -> None:
    """
    Strategiyani yakunlaydi:
    - settings.is_active = FALSE qiladi
    - start_date ni NULL ga tushiradi (yangi strategiya uchun tayyor)

    end_date_str — ixtiyoriy, log uchun ishlatiladi.
    ...
    try:
        async with pool.acquire() as conn:
            ...
            """, user_id)
            logger.info(
                f"Strategiya yakunlandi [user_id={user_id}]"
...
async def clear_strategy_trades(user_id: int) -> None:
    """
    Yangi strategiya boshlananda strategy_trades jadvalini tozalaydi.

    ⚠️ Diqqat: Faqat strategy_trades tozalanadi.
    Asosiy trades jadvali o'zgarmaydi — barcha tarix saqlanadi.
    """
    pool = await get_pool()
    try:
        async with pool.acquire() as conn:
            ...
            """, user_id)
            logger.info(
                f"strategy_trades tozalandi [user_id={user_id}] "
                f"— {deleted} ta trade o'chirildi"
            )
    except Exception as e:
        logger.error(f"clear_strategy_trades xato [user_id={user_id}]: {e}")
        raise


async def clear_daily_journal(user_id: int) -> None:
    """
    Yangi strategiya boshlananda daily_journal jadvalini tozalaydi.

    ⚠️ Diqqat: Faqat shu user ning kunlik jurnali o'chiriladi.
    Boshqa userlarning ma'lumotlariga tegmaydi.
    """
    pool = await get_pool()
    try:
        async with pool.acquire() as conn:
            ...
            """, user_id)
            logger.info(
                f"daily_journal tozalandi [user_id={user_id}] "
                f"— {deleted} ta kun o'chirildi"
            )
    except Exception as e:
        logger.error(f"clear_daily_journal xato [user_id={user_id}]: {e}")
        raise
...
async def get_strategy_summary(
    ...
    start_date_str, end_date_str — ixtiyoriy, sana bo'yicha filtrlash uchun.
    Ko'rsatilmasa — barcha kunlar hisoblanadi.
    ...
    try:
        async with pool.acquire() as conn:
            ...
            # Sana filtri — berilsa ishlatiladi, yo'q bo'lsa barcha kunlar
            from datetime import datetime as dt
            date_filter = "WHERE user_id = $1"
            params: list = [user_id]

            if start_date_str:
                try:
                    d_from = dt.strptime(start_date_str, "%d.%m.%Y").date()
            ...
            if end_date_str:
                try:
                    d_to = dt.strptime(end_date_str, "%d.%m.%Y").date()
                    params.append(d_to)
                    date_filter += f" AND date <= ${len(params)}"
                except ValueError:
                    pass
            ...
            last_balance_row = await conn.fetchrow(f"""
                SELECT end_balance FROM daily_journal
                {date_filter} AND is_completed = TRUE
                ORDER BY date DESC
                LIMIT 1;
            """, *params)
