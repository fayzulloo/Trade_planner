...
async def job_create_daily_journals(bot) -> None:
    ...
    for user in users:
        try:
            ...
            # Jurnal rejimida — maqsadsiz journal yaratiladi
            if settings.get("mode") == "journal":
                # start_date yo'q, day_number doim o'sib boradi
                pool = await get_pool()
                ...
                # end_balance NULL bo'lishi mumkin (yakunlanmagan kun)
                # — bunday holda start_balance, u ham NULL bo'lsa — settings dan olamiz
                if last:
                    start_balance = float(
                        last["end_balance"] or
                        last["start_balance"] or
                        settings.get("starting_balance") or 0
                    )
                else:
...
async def job_morning_reminder(bot) -> None:
    ...
    for user in users:
        try:
            ...
            # Jurnal rejimida — oddiy eslatma
            if settings.get("mode") == "journal":
                await bot.send_message(
                    telegram_id,
                    "📝 <b>Jurnal rejimi</b>\n"
                    "⏰ Savdolaringizni qayd etishni unutmang!",
                    parse_mode="HTML",
                )
                continue
...
async def _handle_strategy_finished(bot, user_id: int, settings: dict) -> None:
    ...
    end_date — oxirgi kun sana (bugun).
    """
    try:
        ...
        await finish_strategy(user_id, end_date_str)
