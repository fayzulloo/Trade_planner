...
from database.queries import get_settings, save_settings, clear_strategy_trades, clear_daily_journal
...
async def _check_and_activate(user_id: int) -> None:
    ...
    Strategy rejimi — starting_balance, start_date, total_days kerak.
    Journal rejimi   — faqat starting_balance kerak.

    Yangi strategiya boshlanmoqda (is_active=FALSE, mode=strategy) bo'lsa —
    strategy_trades va daily_journal tozalanadi.
    Journal rejimida tozalanmaydi.
    """
    settings = await get_settings(user_id)
    if not settings:
        return
    ...
    if mode == "journal":
        # Journal rejimi — faqat balans bo'lsa faollashtirish, hech narsa tozalanmaydi
        if settings.get("starting_balance"):
            await save_settings(user_id, is_active=True)
        return

    # Strategy rejimi — uchta maydon to'liq bo'lishi kerak
    if (
        settings.get("starting_balance") and
        settings.get("start_date") and
        settings.get("total_days")
    ):
        # Yangi strategiya boshlanmoqda — eski ma'lumotlar tozalanadi
        if not settings.get("is_active"):
            await clear_strategy_trades(user_id)
            await clear_daily_journal(user_id)
