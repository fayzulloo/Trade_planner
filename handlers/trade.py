...
async def _get_day_number(user_id: int) -> int | None:
    ...
    Strategy rejimida start_date dan hisoblanadi.
    Journal rejimida bugungi daily_journal dan olinadi.
    """
    ...
    if mode == "journal":
        # Journal rejimida bugungi journal yozuvidan day_number olinadi
        from database.queries import get_today_journal
        journal = await get_today_journal(user_id, today)
        return journal["day_number"] if journal else None
