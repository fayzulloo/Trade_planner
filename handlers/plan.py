...
async def _get_or_create_today_journal(user_id: int, settings: dict):
    ...
    Strategy rejimi: start_date dan day_number hisoblanadi.
    Journal rejimi:  start_date yo'q — bugungi journal dan day_number olinadi.
    ...
    if mode == "journal":
        ...
        if not journal:
            # Scheduler o'tkazib yuborgan — zaxira yaratish
            from database.connection import get_pool
            pool = await get_pool()
            async with pool.acquire() as conn:
                last = await conn.fetchrow("""
                    SELECT day_number, end_balance, start_balance
                    FROM daily_journal
                    WHERE user_id = $1
            ...
            start_balance = float(
                (last["end_balance"] or last["start_balance"]) if last
                else (settings.get("starting_balance") or 0)
            )
        ...
        return journal, journal["day_number"]
...
async def show_plan(message: Message, user_id: int, **kwargs) -> None:
    ...
    try:
        ...
        # Jurnal rejimida — faqat bugungi PnL ko'rsatiladi
        if settings.get("mode") == "journal":
            await _show_journal_mode(message, user_id, settings)
            return
        ...
        if start_date:
            ...
            if is_strategy_finished(
                ...
                summary = await get_strategy_summary(user_id, start_date_str, end_date_str)
                await message.answer(
...
@router.callback_query(F.data == "refresh_journal")
async def refresh_journal(callback: CallbackQuery, user_id: int, **kwargs) -> None:
    """Jurnal rejimini yangilash."""
    try:
        settings = await get_settings(user_id)
        await _show_journal_mode(callback.message, user_id, settings)
    except Exception as e:
        logger.error(f"refresh_journal xato [user_id={user_id}]: {e}")
    finally:
        await callback.answer()


async def _show_journal_mode(message, user_id: int, settings: dict) -> None:
    """
    Jurnal rejimida bugungi holat ko'rsatish.
    Maqsad va strategiya yo'q — faqat bugungi PnL.
    """
    try:
        from database.connection import get_pool
        from utils.calculator import get_current_date
        ...
        pool = await get_pool()
        async with pool.acquire() as conn:
            # Bugungi savdolar yig'indisi (asosiy jurnal)
            ...
            """, user_id, timezone, today)

            # Umumiy balans (barcha vaqt)
            total_row = await conn.fetchrow("""
                SELECT COALESCE(SUM(
                    pnl + COALESCE(swap,0) + COALESCE(commission,0)
                ), 0) AS total_pnl
        ...
        actual_pnl   = float(row["actual_pnl"] or 0)
        trade_count  = int(row["trade_count"] or 0)
        total_pnl    = float(total_row["total_pnl"] or 0)
        balance      = start_bal + total_pnl
        pnl_icon     = "🟢" if net_pnl >= 0 else "🔴"

        from handlers.keyboards import journal_mode_kb
        await message.answer(
            f"📝 <b>Jurnal rejimi</b>\n"
            f"{'─' * 20}\n"
            f"💰 Balans: <b>{format_money(balance)}</b>\n\n"
            f"📅 <b>Bugun:</b>\n"
            f"{pnl_icon} Net PnL: <b>{format_money(net_pnl)}</b>\n"
            f"📊 Savdolar: {trade_count} ta",
            reply_markup=journal_mode_kb(),
            parse_mode="HTML",
        )
    except Exception as e:
        logger.error(f"_show_journal_mode xato [user_id={user_id}]: {e}")
